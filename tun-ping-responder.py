...
def get_tuntap_ComponentId():
    ...
    '''
    with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, ADAPTER_KEY) as adapters:
        try:
            for i in range(10000):
                key_name = winreg.EnumKey(adapters, i)
                with winreg.OpenKey(adapters, key_name) as adapter:
                    try:
...
def openTunTap():
    ...
    configTunParam += [0, 0, 0, 0, 0, 0, 0, 0]
...
class ReadThread(threading.Thread):
    ...
    def _createIpv6EchoReply(self, echoRequest):
        ...
        pseudo += [0x00] * 3 + [len(echoRequest[40:])]
        pseudo += [0x00] * 3                         # zero
...
class WriteThread(threading.Thread):
    ...
    def transmit(self, dataToTransmit):
        ...
        win32file.WriteFile(
            self.tuntap, dataToTransmit.encode(), self.overlappedTx)
        win32event.WaitForSingleObject(
    ...
    def _createIPv4echoRequest(self):
        ...
        echoRequest += [0]                    # code
        # Checksum (to be filled out later)
        echoRequest += [0x00, 0x00]
